...
function FinOrders (){
  const {main, avaFin, setMain, finOrders,active1, setActive1, songsData, setSongsData, HandleDelete2, HandleSetNew} = useCartContext()
          ...
          <div className="main">
            ...
            <div style={{display: `${mobileApp ? `${main ? "flex" : "none"}` : "flex"}`}} className="source">
              <div className="nameText block">
                <p className="name selectUser">{active1.nameAsClient ? active1.nameAsClient : "name"}</p>
                <p className="prg selectUser">{active1.comment}</p>
              </div>
              ...
              <div className="number block selectUser"><span>Номер:</span>{active1.phoneNumber}</div>
